...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.masternode) {
        reward -= rpcData.masternode.amount;
        rewardToPool -= rpcData.masternode.amount;

        txOutputBuffers.push(Buffer.concat({
            util.packInt64LE(rpcData.masternode.amount),
            util.varIntBuffer(rpcData.masternode.script.length),
            rpcData.masternode.script
        ]));
    }

    for (var i = 0; i < recipients.length; i++){
        var recipientReward = Math.floor(recipients[i].percent * 4000000000); // hax
